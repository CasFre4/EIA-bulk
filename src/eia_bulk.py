...
class EIABulk:
    def __init__(self, api_key=None, route=None,
        ...
        host=None, dbname=None, username=None, password=None, tablename=None, write2db=None, override=False, from_api=True, verbose=1, multiplier=None, sleep_between_calls=.4):
        ...
        if os.path.isfile('./route_trees/updated_route_tree.json'):
            file = open('./route_trees/updated_route_tree.json', 'r')
        elif os.path.isfile('./route_trees/og_path_tree.json'):
            file = open('./route_trees/og_path_tree.json', 'r')
        else:
            file = None
        
        if os.path.isfile('./route_trees/updated_short_route_tree.json'):
            short_file = open('./route_trees/updated_short_route_tree.json', 'r')
        elif os.path.isfile('./route_trees/og_path_tree.json'):
            short_file = open('./route_trees/og_short_route_tree.json', 'r')
        else:
            short_file = None

        if file is not None:
            self.all_routes = json.load(file)
        else:
            self.all_routes = {}
        if short_file is not None:
            self.short_all_routes = json.load(short_file)
        else:
            self.all_routes = {}
    ...
    def collect_years_data(self, years=None, start_year=None, end_year=None):
        ...
        for y in years:
            if "hourly" in self.frequency or 'week' in self.frequency or 'daily' in self.frequency:
                yield self.collect_year_small_data(y, engine)
        if 'monthly' in self.frequency or 'quarterly' in self.frequency or 'yearly' in self.frequency:
            
            if len(years) == 1 or all(years[i + 1] - years[i] == 1 for i in range(len(years) - 1)):
                yield self.collect_year_large_data(years, engine)
            else:
                for y in years:
                    yield self.collect_year_large_data([y], engine)
    ...
    def collect_months_data(self, year, months=None, start_month=None, end_month=None):
        ...
        if "hourly" in self.frequency:
            yield self.collect_year_small_data(year, engine, months=months)
        if 'monthly' in self.frequency or 'quarterly' in self.frequency or 'yearly' in self.frequency:
            yield self.collect_year_large_data([year], engine)
        if self.write2db:       
            engine.dispose()
